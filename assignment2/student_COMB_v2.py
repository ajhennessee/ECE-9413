...
Optimized version: Montgomery arithmetic and branchless reductions.
"""
...
# -----------------------------------------------------------------------------
# 32-bit primitives (compulsory) - OPTIMIZED
# -----------------------------------------------------------------------------
...
def mod_add_32(a, b, q):
    """Return (a + b) mod q using branchless selection."""
    a, b, q = jnp.uint32(a), jnp.uint32(b), jnp.uint32(q)
    s = a + b
...
def montgomery_mul_32(a, b, q, q_inv):
    """Return (a * b * R^-1) mod q. Tightened reduction logic."""
    a64, b64 = _to_u64(a), _to_u64(b)
    q64, q_inv64 = _to_u64(q), _to_u64(q_inv)

    t = a64 * b64
    # m = (t mod 2^32) * q_inv mod 2^32
    m = (t.astype(jnp.uint32) * q_inv64.astype(jnp.uint32)).astype(jnp.uint64)

    # u = (t + m*q) >> 32
    u = (t + m * q64) >> 32

    return jnp.where(u >= q64, (u - q64).astype(jnp.uint32), u.astype(jnp.uint32))


# -----------------------------------------------------------------------------
# 64-bit primitives (optional) - OPTIMIZED
# -----------------------------------------------------------------------------
...
def mod_add_64(a, b, q):
    ...
    s = a + b
    # Overflowed if s < a OR if result is simply >= q
    return jnp.where((s < a) | (s >= q), s - q, s)
...
def _mul_64_to_128(a, b):
    a_lo, a_hi = a & _U64_MASK_LO32, a >> _U64_SHIFT32
    b_lo, b_hi = b & _U64_MASK_LO32, b >> _U64_SHIFT32

    ll, lh = a_lo * b_lo, a_lo * b_hi
    hl, hh = a_hi * b_lo, a_hi * b_hi

    mid = lh + hl
    mid_hi = (mid >> _U64_SHIFT32) + jnp.where(mid < lh, _U64_SHIFT32, jnp.uint64(0))
    mid_lo = (mid & _U64_MASK_LO32) << _U64_SHIFT32

    lo = ll + mid_lo
    hi = hh + mid_hi + jnp.where(lo < ll, jnp.uint64(1), jnp.uint64(0))
    return hi, lo


def _reduce_128_mod_q_64(hi, lo, q):
    """Note: This remains slow due to bit-looping. Montgomery is preferred."""
    q = _u64(q)
    r = jnp.uint64(0)
...
def montgomery_mul_64(a, b, q, q_inv):
    ...
    m = lo * _u64(q_inv)
    mq_hi, _ = _mul_64_to_128(m, q)

    # Carry out of lower bits is handled by hi addition + boundary check
    u = hi + mq_hi + jnp.where(lo > 0, jnp.uint64(1), jnp.uint64(0))
    return jnp.where(u >= q, u - q, u)


# -----------------------------------------------------------------------------
# 128-bit primitives (UNCHANGED)
# -----------------------------------------------------------------------------
...
def _reduce_256_mod_q_128(w3, w2, w1, w0, q_hi, q_lo):
    r_hi, r_lo = jnp.uint64(0), jnp.uint64(0)
    ...
    def step(r_hi, r_lo, src_word, bit_idx):
        ...
        return jnp.where(needs_sub, sub_hi, new_hi), jnp.where(needs_sub, sub_lo, new_lo)
...
def mod_add(a, b, q, *, bit_width=32):
    bw = int(bit_width)
    if bw == 32: return mod_add_32(a, b, q)
    if bw == 64: return mod_add_64(a, b, q)
    if bw == 128: return mod_add_128(a, b, q)
    raise ValueError(f"Unsupported bit_width={bit_width}")

def mod_sub(a, b, q, *, bit_width=32):
    bw = int(bit_width)
    if bw == 32: return mod_sub_32(a, b, q)
    if bw == 64: return mod_sub_64(a, b, q)
    if bw == 128: return mod_sub_128(a, b, q)
    raise ValueError(f"Unsupported bit_width={bit_width}")

def mod_mul(a, b, q, *, bit_width=32):
    bw = int(bit_width)
    if bw == 32: return mod_mul_32(a, b, q)
    if bw == 64: return mod_mul_64(a, b, q)
    if bw == 128: return mod_mul_128(a, b, q)
    raise ValueError(f"Unsupported bit_width={bit_width}")
...
def mle_update(zero_eval, one_eval, target_eval, *, q, bit_width=32):
    bw = int(bit_width)
    if bw == 32: return mle_update_32(zero_eval, one_eval, target_eval, q=q)
    if bw == 64: return mle_update_64(zero_eval, one_eval, target_eval, q=q)
    raise ValueError(f"Unsupported bit_width={bit_width}")


# -----------------------------------------------------------------------------
# Sumcheck Kernels - OPTIMIZED ACCUMULATION
# -----------------------------------------------------------------------------
...
def _evaluate_and_sum_stacked_32(expression, t_stacks, key_to_idx, q, q_inv):
    ...
    for term in expression:
        ...
        if acc is None:
            acc = prod.astype(jnp.uint64)
        else:
            acc += prod.astype(jnp.uint64)

    return (acc.sum(axis=-1) % jnp.uint64(q)).astype(jnp.uint32)
...
def sumcheck_32(eval_tables, *, q, expression, challenges, num_rounds):
    ...
    table_stack = jnp.stack([jnp.asarray(eval_tables[k], dtype=jnp.uint32) for k in keys])
    ...
    j_vals = montgomery_mul_32(jnp.arange(degree + 1, dtype=jnp.uint32), R2_mod_q, q, q_inv)

    all_round_evals = []
    for round_idx in range(num_rounds):
        z, o = table_stack[:, ::2], table_stack[:, 1::2]
        ...
        all_round_evals.append(_evaluate_and_sum_stacked_32(expression, t_stacks, key_to_idx, q, q_inv))
    ...
    return montgomery_mul_32(claim0, one_mont, q, q_inv), montgomery_mul_32(all_round_evals, one_mont, q, q_inv)
...
def _evaluate_and_sum_stacked_64(expression, t_stacks, key_to_idx, q, q_inv):
    ...
    for term in expression:
        ...
        if acc is None:
            ...
            acc = mod_add_64(acc, prod, q)

    def add_mod(a, b):
        return mod_add_64(a, b, q)

    return jax.lax.reduce(acc, jnp.uint64(0), add_mod, dimensions=[1])
...
def sumcheck_64(eval_tables, *, q, expression, challenges, num_rounds):
    ...
    table_stack = jnp.stack([jnp.asarray(eval_tables[k], dtype=jnp.uint64) for k in keys])
    ...
    degree = max(len(term) for term in expression)
    j_vals = montgomery_mul_64(jnp.arange(degree + 1, dtype=jnp.uint64), R2_mod_q, q, q_inv)
    ...
    for round_idx in range(num_rounds):
        ...
        all_round_evals.append(_evaluate_and_sum_stacked_64(expression, t_stacks, key_to_idx, q, q_inv))
        r = challenges[round_idx]
    ...
    one = jnp.uint64(1)
    return montgomery_mul_64(claim0, one, q, q_inv), montgomery_mul_64(all_round_evals, one, q, q_inv)
...
def sumcheck(eval_tables, *, q, expression, challenges, num_rounds, bit_width=32):
    ...
    if bw == 64:
        return sumcheck_64(eval_tables, q=q, expression=expression, challenges=challenges, num_rounds=num_rounds)
    raise ValueError(f"Unsupported bit_width={bit_width}")